...
customElements.define("home-head", class extends HTMLElement {
    constructor() { super(); }
    ...
    connectedCallback() {
        ...
        de('HomeHeadRight').addEventListener('mouseenter', () => {
            if (window.innerWidth >= 700 && !de('HomeHead').parentElement.classList.contains('blur')) {
        ...
        de('HomeHeadRight').addEventListener('mouseleave', () => {
            if (window.innerWidth >= 700) {
                de('HomeHeadLeft').style.filter = 'blur(0px)'
                de('HomeHeadLeft').style.opacity = 1;
            }
        ...
        de('HomeHeadNavigationButton').addEventListener('click', () => {
            if (window.innerWidth < 700) {
                if (!HeadNavExpanded) {
                    HeadNavExpanded = true; ds('--HomeNavOpacity', 1); de('HomeHeadLeft').style.opacity = 0.6;
                    ...
                    HeadNavExpanded = false; de('HomeNavLinks').style.opacity = 0;
                    setTimeout(() => { de('HomeNavLinks').style.display = 'none' }, 300)
                    de('HomeHeadLeft').style.filter = 'blur(0px)';
                    de('HomeHeadLeft').style.opacity = 1;
                }
        ...
        <div id="HomeHeadRight">
            <div id="HomeNavigation">
                ...
                <div id="HomeNavLinks" class="fadeChild">
                    ...
                    <p fd="775" onclick="ScrollTo('HomeByme')">By me</p>
                    <p fd="800" onclick="ScrollTo('HomeByme')">About me</p>
                    <p fd="825" onclick="ScrollTo('HomeByme')">Branding</p>
                    <p fd="850" onclick="ScrollTo('HomeByme')">Contact me</p>
                </div>
            </div>
        </div>
...
customElements.define("home-byme", class extends HTMLElement {
    constructor() { super(); }
    connectedCallback() { this.render(); }

    render() {
        this.innerHTML = `<div id="HomeByme">
            <iv- class="fadeSelf" fd="0" id='HomeBymeTitle'></iv-> 
        </div>`
    }
});
