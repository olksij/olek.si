...
import { ComputeAPI, FontsTransmit, ComputeRequest } from '../interfaces';
...
onmessage = (message: MessageEvent<ComputeAPI<'initial'>>) => {
  ...
  if (deliver == 'fonts') fonts.load (request, data as FontsTransmit<'initial'>);
  if (deliver == 'texts') interpolate(request, data as ComputeRequest<'initial'>);
}
